...
def manage_volumes(worker_specification: dict, common_parameters: dict, worker_dict: dict) -> dict:
    ...
    # worker_data_volumes = worker_dict.get('data_volumes', [])
    ...
    if len(data_volumes) > 0:
        ...
        for volume in data_volumes:
            ...
            # data_volume = {device_name: volume}
            worker_dict['data_volumes'][device_name] = volume
            # volume['mount_point'] = device_name
            # worker_data_volumes.append(volume)
            # worker_data_volumes.append(data_volume)
    # worker_dict['data_volumes'] = worker_data_volumes
    # pprint(worker_dict['data_volumes'])
...
def get_worker_type(worker_specification: dict, common_parameters: dict, supported_types:set) -> str:
    vm_worker_type = get_from_dict_with_check(worker_specification, common_parameters, 'worker_type')
    if vm_worker_type not in supported_types:
        raise Exception(f"Worker type {vm_worker_type} is not supported. Supported types are: {', '.join(supported_types)}")
...
class SubnetDispenser:
    ...
    def __init__(self, subnets:list, all_worker_types:list, subnets_tier2:list):
        ...
        for worker_type in all_worker_types:
            self.subents_per_worker_type[worker_type] = subnets if worker_type.lower() != 'forwarder' else subnets_tier2
            self.iterator_per_worker_type[worker_type] = 0


    def next(self, worker_type:str):
...
def get_security_groups(worker_type:str, sg_mapping):
    type_to_key = {} 

    for worker_type_mapping in sg_mapping.keys():
        if worker_type.upper().replace('_','') in worker_type_mapping.replace('-', ''):
            type_to_key[worker_type] = worker_type_mapping
