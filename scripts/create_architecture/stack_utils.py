...
def check_if_aws_is_configured(accout_id):
    ...
    try:
        ...
        if accout_id != ret['Account']:
            l.error(f"Account ID mismatch: {accout_id} != {ret['Account']}")
            exit(1)
...
def wait_until_stack_reaches_status(stack_name:str, desired_status:str = DESIRED_STACK_STATUS):
    retry = 0
...
def display_stack_events(stack_name: str, cloudformation_client):
...
def wait_until_stack_is_created_and_get_outputs(stack_name:str, delete_if_failed:bool = False) -> None:
    cloudformation_client = boto3.client('cloudformation')
    ...
    if stack_status != DESIRED_STACK_STATUS:
        ...
        display_stack_events(stack_name, cloudformation_client)
        l.info("")
